...
OfDialog::OfDialog(wxWindow* parent):wxDialog(parent, -1, _("create a new openframeworks project"), wxDefaultPosition, wxSize(700, 400))
{
	...
	wxString projectPath=dlg->GetPath();
	...
	wxTextCtrl* projPath=new wxTextCtrl(panel, -1, _(""), wxPoint(marginX, curY), wxSize(w1, h));
	wxButton *projPathSelectBtn = new wxButton(panel, -1, wxT("select"), wxPoint(marginX*2+w1, curY), wxSize(w2, h));
	curY+=h+marginY*2;
	...
	wxButton *closeButton = new wxButton(this, -1, wxT("Cancel"), wxDefaultPosition, wxSize(70, 30));

	hbox->Add(okButton, 1);
	hbox->Add(closeButton, 1, wxLEFT | wxRIGHT, 5);
